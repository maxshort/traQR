...
/**
 * Not ours - Modified from http://techdiary.peterbecker.de/2009/02/defending-against-xss-attacks-in.html
 */
public class WrappingReader extends Reader {
    ...
    public int read(char[] cbuf, int off, int len) throws IOException {
        ...
        int oldPos = pos;
        Logger.getLogger(getClass().getName()).trace(String.format("Reading %d characters from position %d", len, pos));
        if (pos < this.prologue.length) {
            final int toCopy = Math.min(this.prologue.length - pos, len);
            Logger.getLogger(getClass().getName()).trace(String.format("Copying %d characters from prologue", toCopy));
            System.arraycopy(this.prologue, pos, cbuf, off, toCopy);
            pos += toCopy;
            if (toCopy == len) {
                Logger.getLogger(getClass().getName()).trace("Copied from prologue only");
                return len;
            }
        }
        if (firstEpilogueChar == -1) {
            ...
            Logger.getLogger(getClass().getName()).trace(String.format("Got %d characters from delegate", read));
            if (read != -1) {
                pos += read;
                if (pos - oldPos == len) {
                    Logger.getLogger(getClass().getName()).trace("We do not reach epilogue");
                    return len;
                }
            }
        ...
        Logger.getLogger(getClass().getName()).trace(String.format("Copying %d characters from epilogue", toCopy));
        System.arraycopy(this.epilogue, epiloguePos, cbuf, off + copiedSoFar, toCopy);
        pos += toCopy;
        Logger.getLogger(getClass().getName()).trace(String.format("Copied %d characters, now at position %d", pos-oldPos, pos));
        return pos - oldPos;
    }
