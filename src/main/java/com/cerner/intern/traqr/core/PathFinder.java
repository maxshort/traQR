...
public class PathFinder {
	...
	public static Trip findPath(Location start, Location end) {
		...
		Map<Location, Connection> previousInShortest = new HashMap<>(); // the
																		// connection
																		// that
																		// connects
																		// the
		...
		while (!queue.isEmpty()) {
			...
			for (Connection connection : location.connections) {
				...
				if (currentBestTimeToEnd == null
						|| timeToLocation.plus(connection.getEstimatedTime()).compareTo(currentBestTimeToEnd) < 0) {
					...
					queue.add(connection.getEnd()); // TODO: need to sort this
													// out so we don't cast
				} else {
	...
	public static Trip extractPath(Location start, Location end, Map<Location, Connection> previousInShortest)
		...
		List<Connection> reversedPath = new ArrayList<>(); // (Note that start
															// and end are _not_
															// reversed w/i
															// individual
															// connections.
		...
		try {
			...
			return new Trip(new ArrayList<Connection>());
		}
		Collections.reverse(reversedPath);
		return new Trip(reversedPath);
	}

	// compares two connected locations based on how long they take from a
	// source as specified by the passed in map
	// only reads from map
	// not thread-safe
	private static class LocationComparator implements Comparator<Location> {
		...
		public int compare(Location loc1, Location loc2) {
			...
			if (loc1Dur == null && loc2Dur == null) {
				...
				return loc1Dur.minus(loc2Dur).getNano(); // TODO: WHY DOES IT
															// ACCEPT THIS??
			}
		}
